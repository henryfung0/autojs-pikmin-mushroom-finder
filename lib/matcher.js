 ...
 * lib/matcher.js
 *
 * Multi-template matching engine supporting multiple template types
 ...
 *   findTemplates(screenImage, templates, config) → match[]
...
/**
 * Fallback search region when no regionFn is provided.
 ...
 *
 * If options.regionFn is provided (e.g. from utils), it is called
 * and its result is used.  Otherwise the built-in fallback region
 * is returned.
 *
 * @param {Object} [options] - Optional configuration.
 * @param {Function} [options.regionFn] - Function returning {x,y,w,h}.
...
/**
 * Find templates in a screenshot by matching against all loaded templates.
 *
 ...
 *   detection sub-section and region function:
 ...
 *   @param {Function} [config.regionFn]
 *     Optional function returning {x,y,w,h} bounding box to restrict
 *     the search region.  If not provided the default screen region
 *     is used.
 ...
 */
function findTemplates(screenImage, templates, config) {
  if (!screenImage) {
    console.warn("findTemplates: screenImage is null or undefined");
    return [];
  }

  if (!templates || templates.length === 0) {
    console.warn("findTemplates: templates array is empty");
    return [];
  }

  // Resolve config values
  var globalThreshold = DEFAULT_MATCH_THRESHOLD;
  var breakOnFirstMatch = false;
  var largeColorThreshold = null;
  var largeElementThreshold = null;

  if (config) {
    if (config.detection) {
      if (config.detection.threshold !== undefined) {
        globalThreshold = config.detection.threshold;
      }
      if (config.detection.largeColorThreshold !== undefined) {
        largeColorThreshold = config.detection.largeColorThreshold;
      }
      if (config.detection.largeElementThreshold !== undefined) {
  ...
  var region = _resolveRegion(config);
  ...
  for (var i = 0; i < templates.length; i++) {
    ...
    // Per-category threshold override — large color / large element can use
    // a lower (more permissive) threshold so they match more easily.
    var effectiveThreshold = globalThreshold;
    if (largeColorThreshold !== null && tpl.name.indexOf("large color") !== -1) {
      effectiveThreshold = largeColorThreshold;
    } else if (largeElementThreshold !== null && tpl.name.indexOf("large element") !== -1) {
      effectiveThreshold = largeElementThreshold;
    }
    ...
    try {
      ...
      if (match) {
        var result = {
          x: match.x,
          y: match.y,
          confidence: match.confidence !== undefined ? match.confidence : effectiveThreshold,
        ...
        if (breakOnFirstMatch) {
          console.info("findTemplates: found '" + tpl.name +
            "' at (" + match.x + ", " + match.y +
      ...
      console.warn("findTemplates: error matching template '" +
        tpl.name + "': " + e);
    }
  }
